...
if not sheet_names:
st.warning("The uploaded workbook does not contain any worksheets.")
st.stop()
...
# Detect important columns safely
...
if not detected:
try:
fallback = detect_business_columns(df)
...
for col, (name, kind) in zip(overview_cols, overview_metrics):
with col:
value = kpis.get(name)
...
```
    if value is None:
        ...
        st.metric(name, f"{float(value):,.2f}")
    elif kind == "percent":
        st.metric(name, f"{float(value):.1f}%")
    else:
        st.metric(name, f"{float(value):,.0f}")
```
...
with signal_cols[0]:
if "Revenue" in kpis and "Profit" in kpis and float(kpis["Revenue"] or 0) != 0:
revenue_value = float(kpis["Revenue"])
profit_value = float(kpis["Profit"])
...
```
    ...
    cost_ratio = (
        cost_total / revenue_total * 100
        if revenue_total
        else 0
    )

    if cost_ratio >= 80:
        st.error(f"🔴 Costs consume {cost_ratio:.1f}% of revenue")
    elif cost_ratio >= 60:
        st.warning(f"🟠 Costs consume {cost_ratio:.1f}% of revenue")
    else:
        st.success(f"🟢 Costs consume {cost_ratio:.1f}% of revenue")
else:
...
and {"Period", "Revenue"}.issubset(time_analysis.columns)
):
trend_chart = px.line(
time_analysis,
x="Period",
...
if len(time_analysis) >= 2:
    first_revenue = float(time_analysis.iloc[0]["Revenue"])
    latest_revenue = float(time_analysis.iloc[-1]["Revenue"])
    ...
    if first_revenue != 0:
        ...
        if change > 0:
            st.success(
                ...
                "from the first available period to the latest available period."
            )
        elif change < 0:
            st.warning(
                f"Revenue changed by {change:.1f}% "
            ...
            st.info(
                "Revenue is unchanged between the first and latest available periods."
            )
```

else:
st.info(
"No usable date + revenue combination was detected for a time trend. "
"Add a date column and a revenue/sales column to unlock this analysis."
)
...
if (
    ...
    and "Revenue" in dimension_analysis.columns
):
    ...
    if top_share >= 70:
        st.warning(
            f"⚠️ Revenue is highly concentrated: "
    ...
    st.info(
        "The selected dimension could not be analyzed with the available data."
    )
```
...
"Columns such as Category, Product, Region, Customer or Department "
"can unlock this analysis."
)
...
if negative_records:
    st.warning(
        f"⚠️ {negative_records:,} record(s) have costs "
        "higher than revenue. These records should be investigated."
    )
else:
    st.success(
        "No record-level losses were detected from Revenue minus Costs."
    )
```
...
if not profit_values.empty:
    ...
    st.info(
        "A direct Profit column was detected, but a Cost column "
        "was not available for full Revenue-vs-Cost analysis."
    )
```

else:
st.info(
"Add Revenue and Cost columns to unlock record-level profitability analysis."
)
...
insights = []
st.warning(
f"Automatic insight generation could not be completed: {exc}"
)
...
if margin < 0:
    recommendations.append(
        "Review loss-making transactions and investigate whether "
        "pricing or costs are driving the negative overall margin."
    )
elif margin < 15:
    recommendations.append(
        "Review pricing, operating costs and low-margin transactions "
        "because overall profit margin is below 15%."
    )
elif margin < 30:
    recommendations.append(
        "Review the largest cost categories and lower-margin transactions "
        "to identify opportunities to improve profitability."
    )
else:
    recommendations.append(
        "Monitor the current margin and identify which products or "
        "categories contribute most to profitable revenue."
    )
```
...
if revenue_total and cost_total / revenue_total > 0.8:
    recommendations.append(
        "Costs are taking a large share of revenue. "
        "Investigate the biggest cost drivers before increasing sales volume."
    )
```
...
```
    if (
        isinstance(recommendation_analysis, pd.DataFrame)
        and not recommendation_analysis.empty
        and "Revenue" in recommendation_analysis.columns
    ):
        total = float(
        ...
        if total:
            ...
            if share >= 50:
                recommendations.append(
                    f"Revenue is concentrated in '{top['Category']}'. "
                    "Review this area closely and consider diversification."
                )
except Exception:
    pass
```
...
if missing_cells:
recommendations.append(
f"Review {missing_cells:,} missing cell(s) before relying "
"on the workbook for final business reporting."
)

if duplicate_rows:
recommendations.append(
f"Review {duplicate_rows:,} duplicate row(s) to confirm "
"they are intentional and not inflating business totals."
)

if not recommendations:
recommendations.append(
"Continue monitoring revenue, costs, profitability and "
"business dimensions as new workbook data is added."
)
...
st.info(
"Revenue or Profit data is required to use Why Analysis."
)
else:
...
if st.button(
    ...
    try:
        ...
        if explanation:
            ...
            st.info(
                "There is not enough information to explain this metric."
            )

    except Exception as exc:
        st.error(
            f"Metric explanation could not be generated: {exc}"
        )
```
